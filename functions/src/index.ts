...
import { duelModules, limitFor, requireDuelModule } from "./duelScope.js";
...
export const startSparring = onCall<StartSparringRequest>(async (request) => {
  ...
  requireDuelModule(moduleId);
  if (!Number.isInteger(level) || level! < 1 || level! > 5) throw new HttpsError("invalid-argument", "Level must be 1–5.");
  const limitMs = limitFor(seconds);

  const modules = await duelModules(moduleId!, [uid]);
  const pool = questionPool(duelLessons.filter((l) => modules.includes(l.moduleId)), Math.random);
