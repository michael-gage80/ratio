        ...
        Group {
            ...
            case .finished:
                if isTutorial {
                    ...
                    if showsDebrief {
                        DuelDebriefView(match: match, result: result, module: module) {
                            dismiss()
                        ...
                        DuelResultView(match: match, result: result, module: module,
                                       rematch: rematch, debrief: { showsDebrief = true }, close: { dismiss() })
                    }
                }
            }
        }
...
private struct DuelResultView: View {
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            ZStack {
                ...
                Circle().fill(result.winner == 0 ? Color.ratioOxblood : Color.ratioSunk).frame(width: 144, height: 144)
                Text("R.").font(.custom("NewsreaderDisplay-Italic", size: 64, relativeTo: .largeTitle))
                    .foregroundStyle(result.winner == 0 ? Color.ratioOnInk : Color.ratioInk)
            }
            .accessibilityHidden(true)
            headline.ratioFont(.display).multilineTextAlignment(.center)
            Text("\(result.score[0]) – \(result.score[1]) · v Sparring partner, level \(match.partner.level)")
                .ratioFont(.monoLabel)
                .foregroundStyle(Color.ratioInk2)
            VStack(spacing: 0) {
                row("Rating · \(module.title)") {
                    let delta = result.ratingAfter - result.ratingBefore
                    Text("\(result.ratingBefore.formatted()) → \(result.ratingAfter.formatted()) \(Text(delta >= 0 ? "+\(delta)" : "\(delta)").foregroundStyle(delta >= 0 ? Color.ratioVerdigris : Color.ratioOxblood))")
                        .ratioFont(.monoData)
                }
                if let fastest {
                    row("Fastest point") {
                ...
                if let moved = result.skillMoved, let skill = Skill(rawValue: moved.skill) {
                    row("Skill moved") {
            ...
            Spacer()
            VStack(spacing: 12) {
                RatioButton("Rematch", action: rematch)
    ...
    private var fastest: (time: Int, answer: String)? {
        result.rounds
            ...
            .flatMap { round in
                match.questions[safe: round.questionIndex].map { (round.answers[0].timeMs, $0.options[$0.correctIndex]) }
            }
    }
